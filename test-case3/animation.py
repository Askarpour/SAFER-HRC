...
def draw_positions(ax, ee, l1, l2, op_head, op_hand, step, task_id):
	...
	ax.add_patch(L_ee)
	#
	L_l1 = lines.Line2D([1000,element_co(l1,'ro')[0]],[700, element_co(l1,'ro')[1]], lw=5., color="red",zorder=2)
	ax.add_line(L_l1)
	#
	L_l2 = lines.Line2D([element_co(l1,'ro')[0], element_co(ee,'ro')[0] ],[element_co(l1,'ro')[1], element_co(ee,'ro')[1]], lw=5., color="red",zorder=2)
	ax.add_line(L_l2)
	#
	return ax.patches
...
def draw_layout(ee, l1, l2, op_head, op_hand, step, task_id):
	...
	ax.set_yticklabels([])
	ax.set_xticklabels([])
	return draw_positions(ax, ee, l1, l2, op_head, op_hand, step, task_id)

def animate(i):
	# for o in animate.trace: o.remove()
	for o in ax.patches: o.remove()
	for o in ax.lines: o.remove()
	for o in ax.lines: o.remove()
	# fig.clear()
	animate.trace  =  draw_layout(EndEffPos[i], Link1Pos[i], Link2Pos[i], BodyPartPosition[1][i], BodyPartPosition[7][i], i, 1)
...
def init():
	...
	# for o in ax.patches: o.remove()
	# for o in ax.lines: o.remove()
	# animate.trace  =  draw_positions(ax,EndEffPos[0], Link1Pos[0], Link2Pos[0], BodyPartPosition[1][0], BodyPartPosition[7][0], 0, 1)
	return  animate.trace,
...
if __name__ == '__main__':
	...
	if os.path.isfile('output.hist.txt'):
		...
		# ax.set_xticklabels([])
		# animate.trace  =  draw_positions(ax,EndEffPos[0], Link1Pos[0], Link2Pos[0], BodyPartPosition[1][0], BodyPartPosition[7][0], 0, 1)
		# for i in range (0, step+1): anim = animation.FuncAnimation(fig, animate, init_func=init,blit = False, repeat=False, frames=step,interval=3000)
		# # plt.show()
		# anim.save(folder+"/Trace.mp4", fps=1.0, extra_args=['-vcodec', 'libx264'])

		images = []
		for filename in os.listdir(folder):
			images.append(imageio.imread(folder+"/"+filename))
			imageio.mimsave(folder+"/Trace2.mp4", images)
	# else:
