...
(defun risk_value_constraints (index value)
	(eval (list `alwf (append `(&&) (loop for hazard_id in index collect `(||

		(|| ,(read-from-string (format nil "(Hazard_Risk_~A= ~A)" hazard_id value)))
))))))	

;;total risk
(defun Risk_estimation ()
 (alw (&&
 	; (->  (Risk= 2) (risk_value_constraints hazard_indexes 2))
 	; (->  (Risk= 1) (risk_value_constraints hazard_indexes 2))
