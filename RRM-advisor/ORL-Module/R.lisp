...
(defun Robot_Structure (roID)
 (eval (list `alwf (append `(&&)
  (loop for i in `(1) collect `(&&
	...
	(always_attached ,(read-from-string (format nil "`Base_~A" roID)) ,(read-from-string (format nil "`Link1_~A" roID)))
	(always_attached ,(read-from-string (format nil "`Link1_~A" roID)) ,(read-from-string (format nil "`Link2_~A" roID)))
	(always_attached ,(read-from-string (format nil "`Link2_~A" roID)) ,(read-from-string (format nil "`EndEff_~A" roID)))
...
(defun 3Dconstraints ()
  (eval (list `alwf (append `(&&)
    (loop for i from (+ 1 L_half) to L_last_up collect
...
(defun always_attached (i j)
  (eval `(alwf 
    (|| 
      ...
      (above_same_L_upper ,(read-from-string (format nil "`~A" i)) ,(read-from-string (format nil "`~A" j)))
